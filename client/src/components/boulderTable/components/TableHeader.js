...
export default function TableHeader ({columns, toggleSortColumn}) {
  ...
  function resetValues(e) {
    const buttons = Array.from(e.target.parentElement.parentElement
  ...
  return (
    <>
      <colgroup>
        ...
        {showColumn('sendAttempts') && <col className='sendAttemptsColumn'>
        </col>}
      ...
      <thead>
        ...
        <tr className='boulderListHead boulderListHeadSelector'>
          ...
          {showColumn('rating') && <th><button value="rating NONE"
            onClick={changeValue}>NONE</button></th> }
          {showColumn('colour') && <th><button value="colour NONE"
            onClick={changeValue}>NONE</button></th>}
          {showColumn('holdType') &&<th><button value="holdType NONE"
            onClick={changeValue}>NONE</button></th>}
          {showColumn('boulderType') && <th><button value="boulderType NONE"
            onClick={changeValue}>NONE</button></th>}
          {showColumn('sendAttempts') && <th><button value="sendAttempts NONE" 
            onClick={changeValue}>NONE</button></th>}
          {showColumn('sendStatus') && <th><button value="sendStatus NONE" 
            onClick={changeValue}>NONE</button></th>}
          {showColumn('startDate') && <th><button value="startDate NONE" 
            onClick={changeValue}>NONE</button></th>}
          {showColumn('sendDate') && <th><button value="sendDate NONE" 
            onClick={changeValue}>NONE</button></th>}
